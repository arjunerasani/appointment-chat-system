...
export default function ChatWindow({ appointmentId, senderType, senderId, senderName, authToken}) {
    ...
    const handleJoinCall = async () => {
        try {
            const response = await fetch(
                `http://localhost:8080/api/livekit/token?appointmentId=${appointmentId}&name=${senderName}`
            );
            ...
            console.error("Failed to acquire LiveKit WebRTC security access token:", error);
        }
    };
    ...
    return (
        <div style={{ display: 'flex', width: '100%', gap: '16px', height: '450px' }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%' }}>
                ...
                {/* input area and action triggers */}
                <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
                    ...
                    {/* Video Call Activation Trigger Inside the Input Row */}
                    {!isMediaActive && (
                        <button onClick={handleJoinCall} disabled={!connected}
                                style={{ padding: '10px 16px', backgroundColor: '#10b981', color: '#fff',
            ...
            {/* LiveKit Active Streaming Panel */}
            {isMediaActive && liveKitToken && (
                <div style={{ width: '350px', display: 'flex', flexDirection: 'column',
                    borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden', backgroundColor: '#1e293b' }}>
                    <LiveKitRoom
                        video={true}
                        audio={true}
                        token={liveKitToken}
                        ...
                        onDisconnected={() => setIsMediaActive(false)}
                        style={{ height: '100%' }}>
                        <VideoConference />
                    </LiveKitRoom>
                </div>
